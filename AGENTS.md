...
- Node 22.14.0 (@.nvmrc). Pre-commit: lefthook (@lefthook.yml — `eslint --fix` + `vitest related` on staged files). CI (@.github/workflows/ci.yml) on push/PR to `main`: the `ci` job runs `astro sync` + lint + unit tests + build + secret scan (build/scan need `SUPABASE_URL`/`SUPABASE_KEY` repo secrets); a parallel `e2e` job boots a local Supabase stack (Docker, `npx supabase start`) and runs the Playwright suite.
...
- **E2E** (`npm run test:e2e`, `e2e/`, Playwright + Chromium): north-star flow (signup → add → dashboard with hand-derived totals), gated-route redirect, and cross-view total consistency in a real browser against the REAL local stack (`npx supabase start` first). Global setup swaps `.dev.vars`/`.env` to local values, boots its own dev server on port 4406, and restores the files after the run. Auth via storageState (`e2e/auth.setup.ts`, fresh `e2e-*` user per run); model new tests on `e2e/seed.spec.ts` and the CLAUDE.md E2E rules; full cookbook in test-plan §6.3. Runs in CI too (`e2e` job: local stack via `npx supabase start`, then `npm run test:e2e`).
- **Secret scan** (`npm run scan:secrets`): builds, then deterministically greps `dist/client/**` for the real `SUPABASE_URL`/`SUPABASE_KEY` values and secret-shaped patterns (`sb_secret_`, `service_role` literal + decoded-JWT payload); exit 2 on a hit, hits name the needle and never print the value (`scripts/scan-secrets.mjs`). Run it after touching env handling, build config, or anything importing `astro:env/server`. CI runs `npm run scan:secrets:dist` (scan only) against the `ci` job's existing build.
- **Post-deploy smoke** (`npm run smoke:prod [-- <base-url>]`, `scripts/post-deploy-smoke.sh`): three curl probes after every `wrangler deploy` — `/` answers 200, `/dashboard` 302-redirects to sign-in, unauthenticated API answers 401; non-zero exit on any failure (test-plan §2 risk #7 — catches the local-pass/prod-dead class).
...
- i18n: every user-facing string goes through `src/lib/i18n.ts` (`translator(lang)`); EN is the default, PL opt-in via the `subtrack-lang` cookie (Topbar toggle). Data values (category/status enums) stay English in the store — only display labels are translated. E2E tests assert the EN strings.
