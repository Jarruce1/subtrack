...
>
> Phase 3 addendum (agent, after coordinator close-out): the 3.6/3.9 "deferred to E2E in M3" caveats are now closed — a headless-Chromium walkthrough (playwright-core, scratchpad-only, 375×812 viewport, fresh user through the real signup/signin forms) verified all 20 checks: signin lands on `/dashboard`; empty state has NO totals block; on `/subscriptions/new` an empty submit renders field errors with zero network POSTs; picking "custom" reveals the required N field and submitting without N is blocked client-side; typing N=3 then switching back to monthly submits clean — exactly one POST, full navigation to `/dashboard` showing PLN 43.00 / month, PLN 516.00 / year, next renewal 2026-08-15 (today 2026-08-08), row stored with `billing_interval_months = null`; no horizontal scroll at 375 px on empty dashboard, add form, and populated dashboard. Curl-side (second user): EUR €9.99/€119.88 and PLN totals render as separate sorted rows; paused custom-3 sub shows its status badge and "Next renewal paused" (no date) with totals unchanged; signout 302 → `/` then `/dashboard` 302 → `/auth/signin`. Env files were pointed at the local stack for the smoke and restored to cloud afterwards. Rendering note: `Intl` money uses a non-breaking space ("PLN 43.00").
